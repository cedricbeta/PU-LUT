...
#         sample_pcd = FPS(input_pcd, input_pcd.shape[2] // 4).squeeze(0)
#         sample_pcd = rearrange(sample_pcd, 'c n -> n c').contiguous()
#         print(sample_pcd.shape)
        
#         sample_pcd_o3d = o3d.geometry.PointCloud()
#         sample_pcd_o3d.points = o3d.utility.Vector3dVector(sample_pcd.cpu().numpy())
        
#         for pt in sample_pcd_o3d.points:
#             [k, idx, _] = pcd_tree.search_knn_vector_3d(pt, 8)
#             input_patch = input_pcd.cpu().numpy()[:, :, idx]
#             input_patch = torch.from_numpy(input_patch).float().cuda()
#             #  # (n, 3) -> (3, n)
#             # input_patch = rearrange(input_patch, 'n c -> c n').contiguous()
#             # # (3, n) -> (1, 3, n)
...
#             # normalize input
#             input_patch, centroid, furthest_distance = normalize_point_cloud(input_patch)
            
#             # upsample
#             upsampled_pcd = pcd_upsample(args, model, input_patch)
#             upsampled_pcd = centroid + upsampled_pcd * furthest_distance
