...
#         pcd_name = path.split('/')[-1]
#         gt_pts = np.array(pcd.points)
#         pts_num = gt_pts.shape[0]
        
#         # For input size with larger size, we need to first divide it into large pathched, each including 1024 points
#         seed_pts = np.asarray(pcd.uniform_down_sample(256).points)
#         upsampled_pcds = np.asarray([[]])
#         for j in range(seed_pts.shape[0]):
#         # for j in range(3):
#             [k, idx, _] = gt_kd.search_knn_vector_3d(seed_pts[j], 1024)
#             input_pcd_patch = np.asarray(gt_pts[idx])
        
        
#             input_pcd = torch.from_numpy(input_pcd_patch).float().cuda()
#             # (n, 3) -> (3, n)
#             input_pcd = rearrange(input_pcd, 'n c -> c n').contiguous()
#             # (3, n) -> (1, 3, n)
#             input_pcd = input_pcd.unsqueeze(0)
...
#             # (b, 3, n) -> (n, 3)
#             # print(upsampled_pcd.shape)
            
#             # upsampled_pcd = rearrange(upsampled_pcd.squeeze(0), 'c n -> n c').contiguous()
#             # upsampled_pcd = upsampled_pcd.detach().cpu().numpy().tolist()
#             if j == 0:
#                 upsampled_pcds = upsampled_pcd
#             else:
#                 upsampled_pcds = torch.cat((upsampled_pcds, upsampled_pcd), dim=2)
    
#         # upsampled_pcds = np.asarray(upsampled_pcds)
#         # upsampled_pcds = rearrange(upsampled_pcds, 'n c -> c n').unsqueeze(0)
#         # upsampled_pcds = FPS(upsampled_pcds, pts_num * args.up_rate)
        
#         print(upsampled_pcds.shape)
#         upsampled_pcds = FPS(upsampled_pcds, pts_num * args.up_rate)
#         upsampled_pcds = rearrange(upsampled_pcds.squeeze(0), 'c n -> n c').contiguous()
#         upsampled_pcds = upsampled_pcds.detach().cpu().numpy()
        
#         # exit()
#         # save path
#         save_path = os.path.join(pcd_dir, pcd_name)
#         np.savetxt(save_path, upsampled_pcds, fmt='%.6f')
#         # time
#         end = time()
#         total_pcd_time += end - start
